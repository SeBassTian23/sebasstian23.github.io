...
class PeriodicTableAA {
  ...
  elementLabel(el) {
    return `<img src="${el.ChemicalStructure}" class="mx-auto d-block mb-3" style="width:auto; height:110px;">`;
  }
  elementInfo(el) {
  ...
  elementLabelSM(el){
    return `<div class="ratio" style="--bs-aspect-ratio: 115%; overflow:hidden; cursor:pointer;">
      <div class="d-flex flex-row-reverse flex-wrap justify-content-between p-1 pt-0">
        ...
        <span style="font-size:0.65rem" class="d-none d-lg-block pt-2"><strong>${el.SingleLetterCode}</strong><br>${el.RelativeMolecularMass}<br>${el["Mz-H2O"]}<br>${el.MolecularFormula}</span>
        <div class="d-none d-lg-block" style="width:100%; height:35%; margin-top: -1rem; background-image:url('${el.ChemicalStructure}'); background-repeat: no-repeat; background-position: center; background-size: contain;""></div>
        <span lang="en" class="fw-bold d-none d-md-block" style="position:absolute; bottom:0; hyphens: auto; text-align: right;">${el.Name}</span>
      </div>
    </div>`;
  }
  table(){
    ...
    for(var i in this.aa.Table.Row){
        ...
        el = this.element(el);
        html += `<td style="background-color:${this.colors[el.GroupBlock]};" data-bs-toggle="modal" data-bs-target="#modal" data-aa="${el.SingleLetterCode}" data-groupblock="${el.GroupBlock}">${this.elementLabelSM(el)}</td>`;
        ...
        if(i == "0"){
          ...
          html += `<td colspan="5"><img alt="" class="d-none d-lg-block mx-auto" style="height:9rem" src="${img}"></td>`;
        }
